...
FILES=(pyproject.toml uv.lock train_unconditional.py maybe_fetch_checkpoint.py train.sh .env)

scp -rP ${PORT} ${FILES[@]} ${HOST}:/workspace

ssh "${REST}" -tp "${PORT}" "${HOST}" "tmux new-session -s diffusion \"cd /workspace && ./train.sh ${TRAIN_ARGS} 2>&1 | tee ./train-output.log\""
